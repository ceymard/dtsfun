...
export const 
  argument = SequenceOf(Optional(T.ellipsis), T.id, T.colon, () => type_literal)
  ...
  generic_arguments = SequenceOf(T.lt, List(() => type_literal, T.comma), T.gt)
  ...
  type_literal: Rule<ast.TypeLiteral> = List(
    Either(
      ...
      SequenceOf(T.lparen, () => type_literal, T.rparen).tf(([lp, type, rp]) => type)
    ),
    T.pipe
